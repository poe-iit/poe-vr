...
namespace Pathfinding {
	...
	public class PathObject : MonoBehaviour
	{
		...
		public int PathLightingSequenceIndex = -1;
	...
	public partial class AIPath : AIBase, IAstarAI {
		...
		protected override void OnDisable()
		{
			...
			/// Change order of lights
			Int3 node_position;
			Collider[] colliders;
			GameObject[] pathGameObjects = null;
			pathGameObjects = GameObject.FindGameObjectsWithTag("path_object");
			for (int count = 0; count < path.path.Count; count++)
			{
				node_position = path.path[count].position;
				if ((colliders = Physics.OverlapSphere((Vector3)node_position, 1f /* Radius */)).Length > 1) //Presuming the object you are testing also has a collider 0 otherwise
				{
					foreach (var collider in colliders)
					{
						...
						Helper.GetAllChildren(pathGameObjects[count].transform, ref children);
						if (children.Contains(go.transform))
						{
							go.GetComponent<PathObject>().PathLightingSequenceIndex = count;            //Do something
						}
					}
				}
